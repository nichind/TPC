...
async def create_dp(tpc) -> None:
    ...
    bot.tpc = tpc
    bot.filters = {"authorized": Authorized, "deauthorized": Deauthorized}

    dp = Dispatcher(bot, storage=MemoryStorage())
    try:
        commands = ["start", "screenshot", "lock", "media"]
        await bot.set_my_commands(
            [
                BotCommand(command, tpc.tl(f"{command}_COMMAND_DESC"))
    ...
    for file in __all__:
        handler = __import__(
            f"core.bot.handlers.{file}", globals(), locals(), ["CurrentInst"], 0
        )
        try:
            ...
            tpc.logger.error(
                f"Handler {file} has no CurrentInst class or setup method in it or it's simply broken... Skipping it"
            )

    try:
        get_me = await bot.get_me()
        tpc.logger.success(
            f"Created Dispatcher for @{get_me.username}, starting polling..."
        )
    except Exception as exc:
    ...
    tpc.pc_handlers.notify(
        tpc.tl("STARTED_BOT"), tpc.tl("STARTED_BOT_TEXT").format(**tpc.bot.chached_me)
    )
    tpc.dp = dp
