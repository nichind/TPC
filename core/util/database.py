...
class Setting(Base):
    ...
    async def add(cls, **kwargs) -> bool:
        async with async_session() as session:
            if (
                await session.execute(select(Setting).filter_by(**kwargs))
            ).scalar() is None:
    ...
    async def get(cls, **kwargs) -> Self:
        ...
        return (
            ...
            else (await cls.get(key=kwargs["key"]) if "key" in kwargs else None)
        )
    ...
    async def update(cls, key: str, value: str) -> Self:
        async with async_session() as session:
            setting = (
                await session.execute(select(Setting).filter_by(key=key))
            ).scalar()
            if setting is None:
